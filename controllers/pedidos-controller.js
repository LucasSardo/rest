...
exports.getpedidos = (req, res, next) => {
    mysql.getConnection((error,conn) => {
        ...
        conn.query(`SELECT pedidos.id_pedido,
                      ...
                      FROM pedidos
                INNER JOIN produtos 
                        ON produtos.id_produto = pedidos.id_produto;`, (error,result,fields)=>{
            if(error){ return res.status(500).send({ error: error }) }
            const response = {
...
exports.postPedidos = (req, res, next) => {
    ...
    mysql.getConnection((error,conn)=>{
        ...
        (error,result,field)=>{
            ...
            if(error){ return res.status(500).send({ error: error }) }
         const response = {
             mensagem: 'Pedido realizado com sucesso com sucesso',
             pedidoCriado: {
...
exports.getPedidoEspecifico = (req, res, next) => {
    mysql.getConnection((error,conn) => {
        if(error){ return res.status(500).send({ error: error }) }
        conn.query('SELECT * FROM pedidos WHERE id_pedido=?;',[req.params.id_pedido], 
        (error,result,fields)=>{
            if(error){ return res.status(500).send({ error: error }) }
            
            if(result.length==0){ 
                return res.status(404).send({mensagem: 'Não foi encontrado pedidos com este ID'
            })
            }
            const response = {
                ...
                pedido: {
                    ...
                    request:{
                        tipo: 'GET',
                        descricao: 'Retorna todos os pedidos',
                        url:'http://localhost:3000/pedidoss'
                    }
                }
            }
            return res.status(200).send({response})
